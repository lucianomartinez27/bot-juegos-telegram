    ...
    async def responder_boton(self, update, context):
        ...
        if not partida_terminada:
            ...
            if t_oculto[int(x)][int(y)] == 9:
                await self.enviar_mensaje(bot, id_usuario, "perdiste bobo")
            ...
            if verificar_tablero(t_visible, bombas):
                descubrir_tablero(t_visible, t_oculto)
                await self.enviar_mensaje(bot, id_usuario, "ganaste bobo")
                self.datos_usuarios[str(id_usuario)]['partida_terminada'] = True
            self.data_manager.save_info(self.datos_usuarios)
            try:
                ...
                await bot.edit_message_reply_markup(chat_id=id_usuario, message_id=id_mensaje,
