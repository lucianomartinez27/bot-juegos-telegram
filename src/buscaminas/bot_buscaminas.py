...
class BotBuscaminas(BotTelegram):
    ...
    def generar_datos(self, id_usuario):
        ...
        self.datos_usuarios[str(id_usuario)]['bombas'] = 5
        t_oculto = self.datos_usuarios[str(id_usuario)]['tablero_oculto'] = crear_tablero(8, 8, 5)
        self.datos_usuarios[str(id_usuario)]['tablero_visible'] = tablero_visible_inicial(t_oculto.copy())
        print(t_oculto)
    ...
    def jugar(self, update, context):
        ...
        opciones = [[InlineKeyboardButton(tablero[i][j], callback_data="{} {}".format(i, j)) for j in range(len(tablero[0]))] \
    ...
    def responder_boton(self, update, context):
        ...
        with open('buscaminas/data.json', 'w') as datafile:
            datafile.write(json.dumps(self.datos_usuarios))
        try:
            keyboard = [[InlineKeyboardButton(t_visible[i][j], callback_data="{} {}".format(i, j)) for j in range(len(t_visible[0]))] \
